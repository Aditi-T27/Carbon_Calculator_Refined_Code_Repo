...
UNION
SELECT hf.feature_name, hf.feature_value, 475 AS result
...
export async function result(){
    const res=
    await pool.query(`SELECT ROUND(SUM(bf.feature_value * fe.emission_coefficient),2) AS total_emission FROM building_features bf JOIN emission_factors fe ON bf.feature_name = fe.feature_name WHERE bf.building_name = 'Building A'`)
    console.log(res[0])
    return res[0];
}
...
export async function Hresult(Bname, HouseNum) {
    const res = await pool.query(
        `SELECT ROUND(SUM(hf.feature_value * fe.emission_coefficient),4) AS total_emission 
         FROM house_features hf 
...
export async function Bresult(Bname){
    const res=
    await pool.query(`SELECT ROUND(SUM(bf.feature_value * fe.emission_coefficient),2) AS total_emission FROM building_features bf JOIN emission_factors fe ON bf.feature_name = fe.feature_name WHERE bf.building_name = ?`,[Bname])
    console.log(res[0])
    return res[0];
}
